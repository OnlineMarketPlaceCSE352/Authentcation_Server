...
public class ChargeCreditHandler {
    ...
    JWTRSA256 keys;
     ...
     public ChargeCreditHandler(Request r, JWTRSA256 keys) {
        this.request = r;
        this.keys = keys;
    ...
    public void Handle() throws ApiException {
        if (request.method.equals(Methods.PUT)) {
            try {
                ...
                try {
                    OTP =request.getBody().getString("OTP");

                }
                catch (JSONException ex)
                {
                    throw new ApiException(Status.BAD_REQUEST,"Missing OTP");
                }
                double newAmount = ChargeCreditServices.Charge(userId,amount,OTP);
